---
name: speckit-bulk-archive-change
...
1. **Get active changes**
   ...
   Run `speckit list --json` to get all active changes.
...
2. **Prompt for change selection**
   ...
   **Load current archive inputs once for the selected root before batch validation:**
   ...
   `speckit instructions archive --change "<selected-change>" --json` with the
...
3. **Batch validation - gather status for all selected changes**
   ...
   a. **Artifact status** - Run `speckit status --change "<name>" --json`
...
7. **Confirm batch operation**
   ...
   `speckit instructions specs --change "<name>" --json` exactly once with the
...
8. **Execute archive for each confirmed change**
   ...
   a. **Sync included delta specs**:
      - Run the `speckit-sync-specs` workflow inline (agent-driven intelligent merge) only for changes with entries in `includedDeltas`, passing only the included delta paths and explicitly instructing it to ignore that change's `excludedDeltas`. Wait for it to finish.
   ...
   b. **Verify included delta specs before moving changeRoot**:
      - Re-run the comparison only for delta specs in `includedDeltas` against main spec at `<planningHome.root>/speckit/specs/<capability-path>/spec.md` (use the store-aware `planningHome.root` from step 3 status JSON, not a hardcoded repo path).
   ...
   c. **Perform the archive**:
      ...
      Target name: use the change name as-is when it already starts with a `YYYY-MM-DD-` prefix; otherwise prepend the current date as `YYYY-MM-DD-<name>` (same rule as `speckit archive`).
...
```text
Conflict: <planningHome.root>/speckit/specs/auth/spec.md touched by [add-oauth, add-jwt]
...
```text
Conflict: <planningHome.root>/speckit/specs/api/spec.md touched by [add-rest-api, add-graphql]
...
- Preserve `.speckit.yaml` when moving to archive
...
- If sync is requested, run the `speckit-sync-specs` workflow inline (agent-driven) for each change with included delta specs
...
- Never archive a change while a spec sync is still in flight — run the sync inline and verify main specs at `<planningHome.root>/speckit/specs/<capability-path>/spec.md` before moving `changeRoot`
