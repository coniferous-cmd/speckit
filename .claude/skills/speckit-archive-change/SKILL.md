...
1. **Select the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/speckit:archive <other>`).

   **Load current archive inputs before the existing archive checks:**

   After resolving the selected change and planning root, run:
   ```bash
   speckit instructions archive --change "<name>" --json
   ```
...
2. **Check artifact completion status**
   ...
   Run `speckit status --change "<name>" --json` to check artifact completion.
...
4. **Assess delta spec sync state**
   ...
   **If delta specs exist:**
   - Compare each delta spec with its corresponding main spec at `<planningHome.root>/speckit/specs/<capability-path>/spec.md` (use the store-aware `planningHome.root` from step 2, not a hardcoded repo path)
   ...
   **Prompt options:**
   ...
   `speckit instructions specs --change "<name>" --json` once with the same
...
5. **Perform the archive**
   ...
   Generate the target name: use the change name as-is when it already starts with a `YYYY-MM-DD-` prefix; otherwise prepend the current date as `YYYY-MM-DD-<change-name>`. Never stack a second date (same rule as `speckit archive`).
...
- Use artifact graph (`speckit status --json`) for completion checking
- Don't block archive on warnings - just inform and confirm
- Preserve `.speckit.yaml` when moving to archive (it moves with the directory)
- Show clear summary of what happened
- If sync is requested, run the `speckit-sync-specs` workflow inline (agent-driven)
