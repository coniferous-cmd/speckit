...
1. **Select the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/speckit:sync <other>`).
...
2. **Resolve change context**
   ...
   The JSON includes `planningHome.root`. Main specs live under `<planningHome.root>/speckit/specs/` — use that (store-aware) root for every main-spec path below, not a hardcoded repo path. When a store is selected it points at the store, not the current repository.
...
4. **For each delta spec, apply changes to main specs**
   ...
   - If archive invoked this workflow inline and supplied a valid snapshot from
     `speckit instructions specs --change "<name>" --json`, reuse it and do not
   ...
   b. **Read the main spec** at `<planningHome.root>/speckit/specs/<capability-path>/spec.md` (may not exist yet)
   ...
   c. **Apply changes intelligently**:
      ...
      **REMOVED Requirements:**
      ...
      - Retiring the capability. Delete the whole `spec.md` - and the directory once
        ...
        5. the change's `.speckit.yaml` declares `retire_capabilities: true`;
      ...
      **`## Purpose` in the delta:**
      - The main spec already has one and it is authoritative - leave it alone
        (this is what `speckit archive` does; it warns and moves on)

   d. **Create new main spec** if capability doesn't exist yet:
      - Create `<planningHome.root>/speckit/specs/<capability-path>/spec.md`
      - Add Purpose section: copy the delta's `## Purpose` body verbatim when it has one
        (this is what `speckit archive` does); only write a brief TBD placeholder when it does not
      - Add Requirements section with the ADDED requirements
      - Follow the **Main Spec Format Reference** below

5. **Validate updated main specs**

   Run `speckit validate --specs` with the same selected-root flags used earlier.
...
- A MODIFIED block carries the whole requirement - body plus every scenario that survives the change. `speckit validate` and `speckit archive` both reject one that drops a scenario the main spec still has.
