---
name: speckit-update-change
description: Update an Speckit change by revising its existing planning artifacts and keeping them coherent with one another. Never edits code.
...
`/speckit:continue` is an optional workflow and may not be installed. Before suggesting it anywhere below, verify that it is available. If it is unavailable, `speckit status --change "<name>" --json` shows the next artifact and `speckit instructions "<artifact-id>" --change "<name>" --json` explains how to create it.
...
1. **Select the change**
   ...
   - If ambiguous, run `speckit list --json` to get available changes sorted by most recently modified, and ask the user to select one
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/speckit:update <other>`).
...
4. **Read and reconcile**
   ...
   - Revise only files that already exist (`existingOutputPaths`). Do NOT create artifacts that don't exist yet, and do NOT invent new files under a glob artifact - note them and point the user to `/speckit:continue` to create them.
...
5. **Confirm and apply, one artifact at a time**
   ...
   - When a substantial rewrite is needed, get that artifact's rules and template first:
     ```bash
     speckit instructions "<artifact-id>" --change "<name>" --json
     ```
...
- Anything deferred to `/speckit:continue` (not-yet-created artifacts or files)
...
- If the request changes the change's *intent* rather than refining it, first verify whether the optional `/speckit:new` workflow is available. If it is, recommend starting fresh with `/speckit:new` (the "Update vs. Start Fresh" heuristic). If it is unavailable, ask for a distinct unused change name and recommend `speckit new change "<new-change-name>"` instead.
